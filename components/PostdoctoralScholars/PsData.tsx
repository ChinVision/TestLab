/*
@author chen
@data 2025--2025/8/5-13:48
*/

export const PsData=[
  {
    src: "/images/people/ps/lqq.jpg",
    name: "Qiqi Liu",
    cname:"刘奇奇",
    research: "",
    experiences: [
    ]
  },
  {
    src: "/images/people/ps/lyk.jpg",
    name: "Yuanke Li",
    cname:"李园珂",
    research: "",
    experiences: [

    ] },
  {
    src: "/images/people/ps/zxy.jpg",
    name: "Xiangyun Zhang",
    cname:"张祥云",
    research: "",
    experiences: [

    ] },
  {
    src: "/images/people/ps/zms.jpg",
    name:"Mingsheng Zhu",
    cname: "朱明盛",
    email:"mingszhu@nankai.edu.cn",
    research: "Nano-biological Effects & Machine Learning",
    experiences: [
      "BSc, 2014-2018, Department of Bioengineering, Huazhong Agricultural University, Wuhan, China.",
      "PhD, 2020-2024, Department of Tissue Engineering and Biomaterials, Nankai University, Tianjin, China",
      'Postdoc, 2024- , Department of Tissue Engineering and Biomaterials, Nankai University, Tianjin, China.',
      "Visiting research fellow, 2024-2025, Department of Diagnostic Radiology, NUS Yong Loo Lin School of Medicine, Singapore."
    ] },
]